...
#include "../DRDX11Renderer/DRDX11Renderer.h"
#else
#include "../DRSoftRenderer/DRSoftRenderer.h"
#endif

bool GameProcess::mResizing = false;

I3DRenderer* GameProcess::m_pDREngineInstance = nullptr;
...
#else
	m_pRenderer = new DRSoftRenderer();
#endif
...
LRESULT CALLBACK GameProcess::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		...
		m_pDREngineInstance->SetClientSize(LOWORD(lParam), HIWORD(lParam));

		if (m_pDREngineInstance->IsValidDevice())
		{
			if (wParam == SIZE_MINIMIZED)
			{
				//mAppPaused = true;
				m_pDREngineInstance->SetWinMinMax(true, false);
			}
			else if (wParam == SIZE_MAXIMIZED)
			{
				//mAppPaused = false;
				m_pDREngineInstance->SetWinMinMax(false, true);
			...
			else if (wParam == SIZE_RESTORED)
			{
				...
				{
					m_pDREngineInstance->SetWinMinMax(false, false);
